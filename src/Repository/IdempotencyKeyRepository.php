...
namespace BlackCat\Database\Packages\IdempotencyKeys\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\IdempotencyKeys\Definitions;
use BlackCat\Database\Packages\IdempotencyKeys\Criteria;
use BlackCat\Database\Packages\IdempotencyKeys\Dto\IdempotencyKeyDto as Dto;
use BlackCat\Database\Packages\IdempotencyKeys\Mapper\IdempotencyKeyDtoMapper as RowMapper;
...
final class IdempotencyKeyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\IdempotencyKeys\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'key_hash' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\IdempotencyKeys\Definitions::table());
            return $bulk->upsertMany($rows, [ 'key_hash' ], [ 'payment_id', 'order_id', 'gateway_payload', 'redirect_url', 'ttl_seconds' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\IdempotencyKeys\Dto\IdempotencyKeyDto|null */
    public function getByKeyHash(string $keyHash, bool $asDto = false): array|\BlackCat\Database\Packages\IdempotencyKeys\Dto\IdempotencyKeyDto|null {
        $row = $this->getByUnique([ 'key_hash' => $keyHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\IdempotencyKeys\Mapper\IdempotencyKeyDtoMapper::fromRow($row);
    }
    public function existsByKeyHash(string $keyHash): bool {
        $where = 't.' . Ident::q($this->db, 'key_hash') . ' = :uniq_key_hash';
        return $this->exists($where, [ 'uniq_key_hash' => $keyHash ]);
    }
    /** @return int|string|null */
    public function getIdByKeyHash(string $keyHash) {
        $row = $this->getByKeyHash($keyHash);
        return $row ? ($row['key_hash'] ?? null) : null;
    }

    /** @return array<string,mixed>|\BlackCat\Database\Packages\IdempotencyKeys\Dto\IdempotencyKeyDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\IdempotencyKeys\Dto\IdempotencyKeyDto|null {
