...
use BlackCat\Database\Packages\IdempotencyKeys\Repository\IdempotencyKeyRepositoryInterface;
...
class IdempotencyKeyRepository implements IdempotencyKeyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'key_hash' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\IdempotencyKeys\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'payment_id', 'order_id', 'gateway_payload', 'redirect_url', 'ttl_seconds' ]);
          return count($rows);
        }
