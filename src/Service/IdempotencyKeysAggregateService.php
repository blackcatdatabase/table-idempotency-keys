 ...
 */
final class IdempotencyKeysAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private IdempotencyKeyRepository $idempotencyKeyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'idempotency_keys' => $this->idempotencyKeyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\IdempotencyKeys\ModuleException("Record {$idStr} not found for locking.");
            }
