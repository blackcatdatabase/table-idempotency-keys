...
final class Definitions {
     ...
     * key_hash may be "id" or "col1, col2".
     * @return string[]
     */
    public static function pkColumns(): array {
        $raw = trim('key_hash');
        if ($raw === '') {
            throw new \InvalidArgumentException('Definitions::pkColumns(): token key_hash must not be empty.');
        }
    ...
    public static function defaultOrder(): ?string {
        $c = trim('created_at DESC, key_hash DESC'); return $c !== '' ? $c : null;
    }

    /** @return array<int,array<int,string>> list of unique keys */
    public static function uniqueKeys(): array { return [ [ 'key_hash' ] ]; }

    /** @return string[] JSON columns for casts/operations */
    public static function jsonColumns(): array { return [ 'gateway_payload' ]; }

    /** @return string[] List of numeric columns (generator heuristic; no runtime DB queries). */
    public static function intColumns(): array { return [ 'tenant_id', 'payment_id', 'order_id', 'ttl_seconds' ]; }
