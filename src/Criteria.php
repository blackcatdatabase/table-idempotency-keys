 ...
 * - whitelist filtrů: [ 'key_hash', 'payment_id', 'order_id', 'gateway_payload', 'redirect_url', 'created_at', 'ttl_seconds' ]
 * - whitelist pro LIKE hledání: [ 'key_hash', 'redirect_url' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'key_hash', 'payment_id', 'order_id', 'gateway_payload', 'redirect_url', 'created_at', 'ttl_seconds' ], true);
        ...
        if ($this->search !== null) {
            $searchCols = [ 'key_hash', 'redirect_url' ];
            $likeParts = [];
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'key_hash', 'payment_id', 'order_id', 'gateway_payload', 'redirect_url', 'created_at', 'ttl_seconds' ], true);
            $parts = [];
