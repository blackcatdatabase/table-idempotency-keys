...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 3 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: todo · PII: 1 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\idempotency-keys](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\idempotency-keys)
...
graph LR
  ...
  idempotency_keys["idempotency_keys"]:::center
  idempotency_keys -->|FK| orders["orders"]:::link
  idempotency_keys -->|FK| payments["payments"]:::accent
  idempotency_keys -->|FK| tenants["tenants"]:::link
...
- Outbound (depends on): "orders", "payments", "tenants"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package idempotency-keys -Apply
